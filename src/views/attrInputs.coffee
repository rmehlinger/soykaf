...
exports.$attrInput = (data, initial, subfield, field="attributes") ->
  metatypeAttr = bind -> metatypeStats[data.getData('priority.metatype')?.metatype]?.attributes?[subfield]
  max = bind ->
    base = metatypeAttr.get()?.limit ? 6
    index = _.findIndex(data.getData('qualities.positive')?.qualia, {name: 'Exceptional Attribute'})
    if index != -1 and data.getData('qualities.positive')?.choice?[index] == subfield
      base += 1
    base
...
exports.$magicAttrInput = (data, initial, subfield) ->
  charMagic = bind -> data.getData('priority.magic')?.attribute
  max = bind ->
    index = _.findIndex(data.getData('qualities.positive')?.qualia, {name: 'Exceptional Attribute'})
    if index != -1 and data.getData('qualities.positive')?.choice?[index] == subfield then 7
    else 6
